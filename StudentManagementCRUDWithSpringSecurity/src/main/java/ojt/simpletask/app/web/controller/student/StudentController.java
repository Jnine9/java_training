...
import ojt.simpletask.app.bl.service.student.UserService;
...
public class StudentController {
	...
	public ModelAndView loginPage() {
		ModelAndView model = new ModelAndView("logins");
		return model;
	}
	...
	public ModelAndView editStudent(@ModelAttribute("student") StudentForm std, @PathVariable("id") Integer id) {
		...
		if (studentService.doGetStudentById(id) == null || !userService.isHasAccess()) {
			model.setViewName("changes");
			if (!userService.isHasAccess())
				model.addObject("changes", "Sorry, only admin can edit student!");
	...
	public ModelAndView deleteStudent(@ModelAttribute("student") StudentForm std, @PathVariable("id") Integer id) {
		ModelAndView model = new ModelAndView();
		if (studentService.doGetStudentById(id) != null && userService.isHasAccess()) {
			model.addObject("changes", "Student <b>" + studentService.doGetStudentById(id).getName()
					+ "</b> has been deleted from our system.");
			model.addObject("systemMessage", messageSource.getMessage("M_SUCCESS_DELETE", null, null));
			studentService.doDeleteStudent(id);
		} else {
			if (!userService.isHasAccess())
				model.addObject("changes", "Sorry, only admin can do this action.");
			else
